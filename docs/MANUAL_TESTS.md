...
4. Проверьте, что нет кнопки template/editor/import/catalog, выбора другой конфигурации,
   default/binding или настройки server commands/parsers.
5. Нажмите F7 и `\` с закрытым GUI, затем с открытым GUI.

Ожидается: global settings сохраняются. F7 и `\` отправляют не более одной Vanilla-box команды
при закрытом GUI и не отправляют её при открытом. Никакой UI route не создаёт, не копирует, не
выбирает, не импортирует и не редактирует альтернативную server configuration.
...
3. На обоих target повторите migration, second-run idempotence, allowed/denied activation и F8.
