...
Сценарии предназначены для проверки сохранения поведения версии 0.5. Используйте тестовый сервер/аккаунт: команды `/pay`, `/mail send`, `/ignoreplayer` и автоответы изменяют серверное состояние.
...
1. На чистом config запустить мод и проверить автоматическую установку bundled `Vanilla-box` и `game` (`mc.vanilla-game.ru`). У `game` должны быть пустые друзья и last seen.
