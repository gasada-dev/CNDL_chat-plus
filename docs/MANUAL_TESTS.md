...
6. Имитировать lookup без ответа: после timeout около 7 секунд тот же друг должен повториться
...
не раньше 10 секунд, после пятого действует пауза 60 секунд, timeout около 7 секунд.
