...
Этот документ описывает планируемую систему серверных шаблонов на основе архитектуры версии 0.4.3. Это проект будущего изменения, а не описание уже реализованной функции. На текущем этапе не меняются Java-классы, JSON-схема, UI, серверные команды и runtime-поведение.
...
Активный шаблон выбирается для текущего соединения. Runtime-подсистемы должны получать один согласованный snapshot активного шаблона, а сохранение из UI — всегда иметь явный целевой идентификатор. Имя шаблона предназначено для пользователя и может изменяться; привязки не должны ссылаться на имя.

Конкретная JSON-схема и версия схемы определяются отдельным PR с миграционными тестами. Имя файла остаётся `.minecraft/config/gasada-chat-responder.json`.
...
Выбор выполняется в порядке:

1. Явная точная привязка нормализованного `host:port`.
2. Явно настроенный fallback/default template.
3. Безопасное состояние «шаблон не выбран», если fallback отсутствует.

Произвольные wildcard-привязки по умолчанию не нужны: слишком широкое совпадение может активировать команды или автоответы не на том сервере. Singleplayer, Realms и соединение без достоверного адреса должны иметь отдельный устойчивый ключ либо оставаться без шаблона; нельзя молча использовать последний шаблон другого соединения.
...
Миграция выполняется отдельным версионированным шагом и должна быть идемпотентной:
...
6. Сохранить recoverable backup старого файла, затем выполнить существующую схему `temporary file -> move -> основной файл`.
