...
## Пользовательские вкладки чата

Редактируемый `outgoingPrefix` пользовательской вкладки является отдельным глобальным UI route.
Когда такая вкладка активна, текст без начального `/` отправляется как команда с её префиксом
через `OutgoingChatService` и проходит те же проверки gate, connection, длины и generation.
Например, вкладка «ГС чат» по умолчанию отправляет текст через `/gc`. Явные slash-команды
остаются vanilla command path и не получают пользовательский префикс.

Этот маршрут не открывает fixed Vanilla-box command templates, placeholders или parsers для
редактирования и не выбирает другую серверную конфигурацию.
