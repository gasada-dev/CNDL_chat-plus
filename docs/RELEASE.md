...
одновременно обновляются `README.md`, `CHANGELOG.md` и `UPDATE_NOTES.md`.
...
2. Увеличить `mod_version` в `gradle.properties`, badge/ссылки в `README.md`,
   раздел версии в `CHANGELOG.md` и краткий список текущей версии в `UPDATE_NOTES.md`.
...
5. Убедиться, что `UPDATE_NOTES.md` непустой, не превышает 4096 байт и содержит только
   актуальные пользовательские изменения текущей версии.
...
После commit и ручной проверки убедиться, что outgoing commits изменяют `UPDATE_NOTES.md`.
Без этого `git push` запрещён.
...
`gh release create --verify-tag --notes-file UPDATE_NOTES.md --latest`. Если tag не равен
...
release body берётся из `UPDATE_NOTES.md`, а полная ссылка ведёт на файл точного тега.
