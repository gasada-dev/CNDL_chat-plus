...
public final class UpdateChecker {
	...
	private static String currentModVersion() {
		return FabricLoader.getInstance().getModContainer(CndlChatPlusClient.MOD_ID)
	...
	private void start() {
		...
		client.sendAsync(request, HttpResponse.BodyHandlers.ofInputStream())
				...
				.exceptionally(exception -> {
					...
					CndlChatPlusClient.LOGGER.debug("Проверка обновления недоступна", exception);
					return null;
				});
	}
	...
	private void handleResponse(HttpResponse<InputStream> response) {
		...
		try (InputStream body = response.body()) {
			...
			state.set(CheckState.FAILED);
			CndlChatPlusClient.LOGGER.debug("Некорректный ответ GitHub Release", exception);
		}
	}
	...
	private static void closeQuietly(InputStream stream) {
		try {
			...
			CndlChatPlusClient.LOGGER.debug("Не удалось закрыть update response body", exception);
		}
	}
