...
6. Каждая версия обновляет `UPDATE_NOTES.md`; его краткие пункты показываются в update screen.
...
- `UPDATE_NOTES.md` редактирует только пользователь. Агенту запрещено создавать, изменять
  или удалять этот файл; разрешены только чтение и проверка его состояния.
- Перед любым `git push` проверьте outgoing commits. Если среди них нет изменения
  `UPDATE_NOTES.md`, откажитесь выполнять push до обновления файла.
